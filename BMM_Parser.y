  ...
  %token  PRINT LET REM EQUALS VARIABLE_NAME NUMBER PLUS MINUS DIVIDE MULTIPLY OPEN_BRACKET CLOSE_BRACKET LESS BIGGER LESSEQ BIGEQ IF THEN AND OR SEMICOLON COMMA STOP RETURN STRING1 GOTO GOSUB DIM END INPUT DEF_FN NOTEQUALS FOR NEXT STEP TO DATA NOT XOR POWER HASH PERCENTAGE DOLLAR EXCLAMATION END1
...
data:
  ...
  for:
      ...
      yyerror("Wrong For Loop Syntax, Next should be followed by same Variable as followed by FOR");}}
      |NUMBER FOR variable_name EQUALS expr TO expr stmt stmt stmt stmt NUMBER NEXT variable_name   {fprintf(f2,"for compile\n\n"); $$=$1; if($3!=$14){
      yyerror("Wrong For Loop Syntax, Next should be followed by same Variable as followed by FOR");}}
      ...
      yyerror("Wrong For Loop Syntax, Next should be followed by same Variable as followed by FOR");} }
      | NUMBER FOR variable_name EQUALS expr TO expr STEP expr stmt stmt stmt stmt NUMBER NEXT variable_name  {fprintf(f2,"for compile\n\n"); $$=$1;if($3!=$16){
      yyerror("Wrong For Loop Syntax, Next should be followed by same Variable as followed by FOR");} }
      ; 
  ...
  VAR :
    ...
    | NUMBER END1 {fprintf(f2,"end compile\n\n"); yyerror("END Statement is not at last"); $$=$1; return 0;}
    ;
  dim :
  ...
  print_expr : 
    ...
    | expr delimeters print_expr { fprintf(f2, "expr1 compile\n\n"); }
    ;
